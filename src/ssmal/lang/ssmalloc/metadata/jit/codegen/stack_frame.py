import ast
from collections import OrderedDict
from typing import Callable, Generator, Literal
from ssmal.lang.ssmalloc.metadata.jit.codegen.compiler_error import CompilerError
from ssmal.lang.ssmalloc.metadata.jit.codegen.compiler_internals import CompilerInternals

TEvaluateCallback = Callable[[ast.expr], Generator[str | bytes, None, None]]


class Scope:
    """Does not check syntax, does not check types
    
    Names:
    names are gathered from the args and AnnAssign with Name targets.
    """

    function_def: ast.FunctionDef
    offsets: OrderedDict[str, int]
    locals: set[str]
    args: set[str]

    def __init__(self, function_def: ast.FunctionDef) -> None:
        self.function_def = function_def
        self.offsets = OrderedDict[str, int]()
        self._parse(self.function_def)

    # fmt: off

    def _check_name(self, name: str, check: Literal['free']) -> bool:
        match check:
            case 'free':
                if name in self.offsets: raise CompilerError('fail',check,name, self)
    
    def _add_name(self, name: str, type: Literal['arg', 'local'], check: Literal['free'] = None) -> None:
        if check is not None:
            self._check_name(name, check)
        self.offsets[name] = len(self.offsets)
        if type == 'arg':
            self.args.add(name)
        else:
            self.locals.add(name)


    def _parse(self, node: ast.AST) -> None:
        match node:
            case ast.FunctionDef(args=args, body=body):
                for arg in args.args:
                    self._add_name(arg.arg, 'local', check='free')
                for stmt in body:
                    self._parse(stmt)
            case ast.AnnAssign(target=ast.Name(id=_id)):
                self._add_name(_id, 'local', check='free')

    # fmt: on

    def get_offset(self, name: str) -> int:
        return self.offsets[name]


# fmt: off

class CallingConvention:
    ci = CompilerInternals()
    evaluate: TEvaluateCallback
    scope: Scope

    def __init__(self, evaluate_callback: TEvaluateCallback, scope: Scope) -> None:
        self.evaluate = evaluate_callback
        self.scope = scope


    def call(self, self_expr: ast.expr, method_name: str, args_exprs: list[ast.expr]) -> Generator[str | bytes, None, None]:
        ci = self.ci
        yield ci.SWPAI; ci.PSHA; yield ci.SWPAI
        yield ci.SWPAS; yield ci.PSHA; yield ci.SWPAS; yield ci.POPA; yield ci.SWPAB
        yield from self.evaluate(self_expr); yield ci.PSHA
        for arg_expr in args_exprs:
            yield from self.evaluate(arg_expr); yield ci.PSHA
        # stack:
        # ret, self, arg1, arg2, ..., argn, TOP
        #      ^                             ^
        #      B                            SP
        # registers:
        # B = &args[0]

        yield ci.LDAb; yield ci.PSHA; yield ci.LDAi; yield ci.ZSTR(method_name)
        # stack:
        # ret, self, arg1, arg2, ..., argn, self, TOP
        #      ^                                   ^
        #      B                                  SP
        # registers:
        # A = &method_name
        # B = &args[0]
        
        yield ci.BRi; yield "Reflection.LoadMethodByName"
        # stack:
        # ret, self, arg1, arg2, ..., argn, TOP
        #      ^                             ^
        #      B                            SP
        # A = method_address
        # B = &args[0]
        yield ci.BRa
    
    def _copy_SP_to_A(self) -> Generator[str | bytes, None, None]:
        ci = self.ci
        yield ci.SWPAS; yield ci.PSHA; yield ci.SWPAS; yield ci.POPA;
    
    def answer(self):
        ci = self.ci
        yield from (ci.PSHA for _ in range(len(self.scope.locals)))


    def return_(self):
        ci = self.ci
        yield from (ci.POPA for _ in range(len(self.scope.locals)))
        yield from (ci.POPA for _ in range(len(self.scope.args)))
        yield ci.POPA; ci.SWPAI


