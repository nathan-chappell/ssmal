...
def test_opcode_map():
    _all_ops = 'nop dbg halt addb subb mulb divb addi subi muli divi add_ sub_ mul_ div_ ldai ldab lda_ stai stab sta_ psha popa cali cala cal_ retn sys swpab swpai swpas bri bra brzi brni brzb brnb'
...
@pytest.mark.parametrize(
    "input,error",
    [
        (""" 1234 foo: bri $bar nop nop""", UnresolvedLabelError),
    ],
)
